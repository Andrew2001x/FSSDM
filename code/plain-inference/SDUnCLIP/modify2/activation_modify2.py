...
class SiLU(Module):
    ...
    def __init__(self, inplace: bool = False) -> None:
        ...
        #
        # NOTE:
        # To stay consistent with the paper's unified gate-form description,
        # SiLU is implemented as x * approx_sigmoid(x), rather than directly
        # fitting SiLU(x) itself.
    ...
    def forward(self, input: Tensor) -> Tensor:
        ...
        cO = self._sigmoid_c_outer.to(device=xs_f.device, dtype=xs_f.dtype)
        cI = self._sigmoid_c_inner.to(device=xs_f.device, dtype=xs_f.dtype)

        # masks on xs
        # (-inf,-7), [-7,-2.2), [-2.2,0), [0,2.2), [2.2,7), [7,inf)
        mL = xs_f < -7.0
