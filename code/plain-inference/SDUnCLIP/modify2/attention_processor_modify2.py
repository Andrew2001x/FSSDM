...
def _poly5_exp_half(u: Tensor) -> Tensor:
    c0 = 1.0
...
def _approx_exp_sqrtexp_deg5(x: Tensor) -> Tensor:
    u = 0.5 * x
    y = _poly5_exp_half(u)
    return y * y
...
def _recip_psln_core(
    t: Tensor,
    alpha: float = _PSLN_ALPHA,
    a: float = _PSLN_A,
    b: float = _PSLN_B,
    eps: float = _EPS,
) -> Tensor:
...
def _reciprocal_psln_positive(denom: Tensor, eps: float = _EPS) -> Tensor:
...
def custom_softmax(input: Tensor, dim: int = -1) -> Tensor:
    x_max = input.amax(dim=dim, keepdim=True)
    x = input - x_max

    need_fp32 = x.dtype in (torch.float16, torch.bfloat16)
    xf = x.float() if need_fp32 else x

    exp_x = _approx_exp_sqrtexp_deg5(xf)
    sum_exp = exp_x.sum(dim=dim, keepdim=True)
    inv_sum = _reciprocal_psln_positive(sum_exp)
    exp_x = exp_x * inv_sum

    if need_fp32:
        exp_x = exp_x.to(dtype=input.dtype)
