...
class SiLU(Module):
    __constants__ = ["inplace", "compute_fp32"]
    inplace: bool
    compute_fp32: bool

    def __init__(self, inplace: bool = True, compute_fp32: bool = False) -> None:
        super().__init__()
        self.inplace = inplace
    ...
    def forward(self, input: Tensor) -> Tensor:
        ...
        # 强烈建议走 inplace（否则会 clone 一份巨大的 out）
        out = x if self.inplace else x.clone()

        if mL.any():
            out.masked_fill_(mL, 0.0)

        def _maybe_fp32(t: Tensor) -> Tensor:
            if self.compute_fp32 and t.dtype in (torch.float16, torch.bfloat16):
                return t.float()
            return t

        if mA.any():
            xA = _maybe_fp32(x[mA])
            a = self._a_coeffs.to(device=xA.device, dtype=xA.dtype)
            segA = self._poly2_horner(xA, a)
            out[mA] = segA.to(dtype=out.dtype)

        if mB.any():
            xB = _maybe_fp32(x[mB])
            b = self._b_coeffs.to(device=xB.device, dtype=xB.dtype)
            segB = self._poly6_sparse(xB, b)
            out[mB] = segB.to(dtype=out.dtype)
    ...
    def extra_repr(self) -> str:
        s = []
        if self.inplace:
            s.append("inplace=True")
        if self.compute_fp32:
            s.append("compute_fp32=True")
        return ", ".join(s)
