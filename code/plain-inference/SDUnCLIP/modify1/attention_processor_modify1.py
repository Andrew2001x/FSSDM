...
def _transform_x(x: Tensor, x_min: float = -14.0, x_max: float = 0.0) -> Tensor:
    # map x from [x_min, x_max] -> [-1, 1]
    return 2.0 * (x - x_min) / (x_max - x_min) - 1.0


def _chexp(x: Tensor) -> Tensor:
    """
    Chebyshev-like polynomial approximation of exp(x) on x in [-14, 0].
    """
    xt = _transform_x(x)

    # Chebyshev recurrence:
    # T0=1, T1=xt, Tn = 2*xt*T_{n-1} - T_{n-2}
    t0 = torch.ones_like(xt)
    t1 = xt

    # coefficients for T0..T7
    c0 = 0.14021878
    c1 = 0.27541278
    c2 = 0.22122865
    c3 = 0.14934221
    ...
    t2 = two_xt * t_nm1 - t_nm2
    ex = ex + c2 * t2
    t_nm2, t_nm1 = t_nm1, t2

    t3 = two_xt * t_nm1 - t_nm2
    ex = ex + c3 * t3
    t_nm2, t_nm1 = t_nm1, t3

    t4 = two_xt * t_nm1 - t_nm2
    ex = ex + c4 * t4
    t_nm2, t_nm1 = t_nm1, t4

    t5 = two_xt * t_nm1 - t_nm2
    ex = ex + c5 * t5
    t_nm2, t_nm1 = t_nm1, t5

    t6 = two_xt * t_nm1 - t_nm2
    ex = ex + c6 * t6
    t_nm2, t_nm1 = t_nm1, t6

    t7 = two_xt * t_nm1 - t_nm2
    ex = ex + c7 * t7
...
def reciprocal_goldschmidt_real(
    x: Tensor,
    eps: float = 1e-12,
) -> Tensor:
    """
    Plaintext reference aligned with current modify1all-style secure reciprocal.

    Behavior:
    - sign/abs/clamp
    - normalize to c in [0.5, 1) using frexp/ldexp
    - Goldschmidt with k = 2
    - only one e = e * e between the two r updates
    """
    sign = torch.where(x >= 0, torch.ones_like(x), -torch.ones_like(x))
    x_abs = torch.abs(x).clamp_min(eps)

    # x_abs = mantissa * 2**exponent, mantissa in [0.5, 1)
    mantissa, exponent = torch.frexp(x_abs)

    # factor = 2^(-exponent), so c = x_abs * factor = mantissa in [0.5, 1)
    ...
    # k = 2, unrolled to match current C++ behavior
    r = r * (1.0 + e)
    e = e * e
    r = r * (1.0 + e)
...
def custom_softmax(input: Tensor, dim: int = -1, recip_iters: int = 2) -> Tensor:
    """
    Softmax using:
      1) subtract max for stability
      2) approximate exp(x) on [-14,0] with Chebyshev polynomial
      3) clip exp on x <= -14 to 0
      4) normalize via Goldschmidt reciprocal approximation
    """
    x_max = input.amax(dim=dim, keepdim=True)
    x = input - x_max

    exp_x = _chexp(x)

    # clip: x <= -14 => 0
    exp_x.masked_fill_(x <= -14.0, 0.0)

    sum_exp = exp_x.sum(dim=dim, keepdim=True)

    # approximate reciprocal of denominator
    inv_sum_exp = reciprocal_goldschmidt_real(sum_exp, num_iters=recip_iters)

    # normalize by multiplication instead of division
    exp_x.mul_(inv_sum_exp)
    return exp_x
