...
namespace shark
{
    namespace crypto
    {
        ...
        inline u8 bit_at(const u128 &x, int idx_from_lsb)
        {
            return static_cast<u8>((x >> idx_from_lsb) & u128(1));
        }

        inline u128 mask_bits_u128(int bin)
        {
            if (bin >= 128)
                return ~u128(0);
            return (u128(1) << bin) - 1;
        ...
        std::pair<DPFRingKey, DPFRingKey> dpfring_gen(int bin, const u128 alpha)
        {
            u128 payload_ring = 1;
            ...
            for (int i = 0; i < bin; ++i)
            {
                const u8 keep = bit_at(alpha, bin - 1 - i);
                auto a = toBlock(keep);
        ...
        std::tuple<u128, u128> do_leaf(int party, const DPFRingKey &key, block s, const std::vector<u64> &lut, u128 x, u128 lut_offset, int bin)
        {
            u8 t = lsb(s);
            ...
            u64 idx = (u64)((x + lut_offset) & mask_bits_u128(bin));
            return std::make_tuple(s_converted_ring * lut[idx], s_converted_tag * lut[idx]);
        }

        std::tuple<u128, u128> do_subtree(int party, const DPFRingKey &key, const std::vector<u64> &lut, int i, int bin, u128 curr_x, block s, u128 lut_offset)
        {
            if (i == bin)
            {
        ...
        std::tuple<u128, u128> dpfring_evalall_reduce(int party, const DPFRingKey &key, const std::vector<u64> &lut, u128 lut_offset)
        {
