...
namespace shark
{
    namespace protocols
    {
        ...
        static std::string currentBatchCheckContext;
        ...
        static inline void fill_random_blocks_parallel(std::vector<block> &seeds)
        {
            fill_with_parallel_prng((u64)seeds.size(), [&](u64 i)
            {
                seeds[(size_t)i] = rand<block>();
            });
        }
        ...
        static void send_dcfbit_impl(const shark::span<T> &share, int bin)
        {
            ...
            fill_random_blocks_parallel(seeds);
        ...
        T commit_and_exchange(T &x)
        {
            ...
            for (int i = 0; i < osuCrypto::RandomOracle::HashSize; i++)
            {
                ...
                {
                    printf("[commit_and_exchange] mismatch party=%d id=%llu idx=%d sizeof(T)=%zu bytesSent=%llu bytesRecv=%llu\n",
                           party,
                    ...
                    const unsigned char *peer_bytes = reinterpret_cast<const unsigned char *>(&peer_x);
        ...
        void set_debug_batchcheck_context(const char *label)
        {
            currentBatchCheckContext = (label != nullptr) ? label : "<null>";
        }
        ...
        void batch_check()
        {
            ...
            if (debug_break_on_mismatch_enabled())
            {
                ...
                {
                    const char *ctx = currentBatchCheckContext.empty()
                        ? "<unset>"
                        : currentBatchCheckContext.c_str();
                    printf("[debug] batch_check context=%s\n", ctx);
