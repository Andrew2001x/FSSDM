...
namespace shark
{
    namespace crypto
    {
        ...
        std::pair<DCFRingKey, DCFRingKey> dcfring_gen(int bin, const u128 alpha, const bool greaterThan)
        {
        ...
        std::tuple<block, u128, u128> traversePathDCF(int party, const DCFRingKey &key, u128 x, const bool geq)
        {
            ...
            for (int i = 0; i < bin; ++i)
            {
                const u8 keep = bit_at(x, bin - 1 - i);
        ...
        std::tuple<u128, u128> dcfring_eval(int party, const DCFRingKey &key, const u128 &x, const bool greaterThan)
        {
