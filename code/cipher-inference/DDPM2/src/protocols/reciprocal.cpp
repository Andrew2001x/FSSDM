    ...
    namespace protocols
    {
        namespace reciprocal
        {
            ...
            void gen(const shark::span<u64> &X, shark::span<u64> &Y, int f)
            {
                ...
                {
                    const int shift = (int)f_p - f;
                    auto tmp = ars::call(Y, shift);
                    #pragma omp parallel for
                    for (u64 i = 0; i < Y.size(); i++)
                    {
                        Y[i] = tmp[i];
            ...
            void eval(const shark::span<u64> &X, shark::span<u64> &Y, int f)
            {
                ...
                for (u64 i = 0; i < X.size(); i++)
                {
                    ...
                    for (u64 j = 0; j < m; j++)
                    {
                        ...
                        {
                            s += 1 * party;
                            s_tag = mac_add(s_tag, mac_mul_u64(1));
                        }

                        u64 term_share = 0;
                        u128 term_tag = 0;
                        auth_local_mul_public_u64(getLow(s), s_tag, (1ull << (63 - j)), term_share, term_tag);
                        auth_local_add_u64(m_scale_share[i], m_scale_tag[i], term_share, term_tag,
                                           m_scale_share[i], m_scale_tag[i]);

                        auth_local_mul_public_u64(getLow(s), s_tag, (1ull << (2*f - j + n_m)), term_share, term_tag);
                        auth_local_add_u64(e_scale_share[i], e_scale_tag[i], term_share, term_tag,
                                           e_scale_share[i], e_scale_tag[i]);
