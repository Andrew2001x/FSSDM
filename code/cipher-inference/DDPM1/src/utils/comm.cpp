...
namespace shark
{
    ...
    void SocketBuf::read(char *buf, u64 bytes)
    {
        ...
        while (total < bytes)
        {
            ssize_t r = recv(recvsocket, (char *)buf + total, bytes - total, MSG_WAITALL);
            if (r < 0)
            {
                if (errno == EINTR)
                    continue;
    ...
    char *SocketBuf::read(u64 bytes)
    {
        ...
        while (total < bytes)
        {
            ssize_t r = recv(recvsocket, (char *)tmpBuf + total, bytes - total, MSG_WAITALL);
            if (r < 0)
            {
                if (errno == EINTR)
                    continue;
    ...
    void SocketBuf::write(char *buf, u64 bytes)
    {
        ...
        while (total < bytes)
        {
            ssize_t w = send(sendsocket, buf + total, bytes - total, 0);
            if (w < 0)
            {
                if (errno == EINTR)
                    continue;
