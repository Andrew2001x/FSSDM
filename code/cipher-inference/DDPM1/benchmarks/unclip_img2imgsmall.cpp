...
static void keygen_configure_progress(int num_inference_steps) {
    ...
    keygen_layer_total[1] = 4;  // image_encoder, unet_core, unet_mid_attn, vae
    keygen_layer_total[2] = 4;  // prompt, image, noise, latents
    keygen_layer_total[3] = 3;  // patch_conv, token_ln, cls_proj
...
int main(int argc, char **argv) {
    ...
    const u64 batch = 1;
    // Use grayscale 4x4 IO while keeping the rest of the model pipeline unchanged.
    const u64 img_H = 4;
    const u64 img_W = 4;
    const u64 C = 1;
