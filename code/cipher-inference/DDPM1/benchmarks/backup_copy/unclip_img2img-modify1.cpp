...
// SiLU (piecewise 5th-order polynomial approximation with symmetry on positive side).
static AuthShare silu_apply(const AuthShare &x) {
    auto add_const = [&](const AuthShare &src, double v) {
        auto c = make_public_const(src.share.size(), v, f);
        return ADD_CALL(src, c);
    ...
    // 5th-order polynomial coefficients for SiLU(x)=x*sigmoid(x).
    // outer: x in [-7, -2.2), inner: x in [-2.2, 0)
    auto cO0 = make_public_const(x.share.size(), -3.24570114e-01, f);
    auto cO1 = make_public_const(x.share.size(), 1.00306737e-01, f);
    auto cO2 = make_public_const(x.share.size(), 1.23002598e-01, f);
    auto cO3 = make_public_const(x.share.size(), 3.25959390e-02, f);
    auto cO4 = make_public_const(x.share.size(), 3.60808855e-03, f);
    auto cO5 = make_public_const(x.share.size(), 1.48142321e-04, f);

    auto cI0 = make_public_const(x.share.size(), 7.17527925e-05, f);
    auto cI1 = make_public_const(x.share.size(), 5.01226523e-01, f);
    auto cI2 = make_public_const(x.share.size(), 2.54580274e-01, f);
    auto cI3 = make_public_const(x.share.size(), 5.36077284e-03, f);
    auto cI4 = make_public_const(x.share.size(), -2.08367638e-02, f);
    auto cI5 = make_public_const(x.share.size(), -3.80900179e-03, f);
    ...
    auto segA = P_outer;
    auto segB = P_inner;
    auto segC = ADD_CALL(x, P_inner_neg);
    auto segD = ADD_CALL(x, P_outer_neg);
    auto segR = x;
...
[[maybe_unused]] static AuthShare ss_reciprocal(const AuthShare &x) {
...
// Softmax using deg-5 sqrt-exp approximation + 2-step Newton inverse.
...
static AuthShare clip_softmax(u64 a, u64 b, const AuthShare &x) {
    ...
    // Step 2: exp(x) via sqrt-exp deg-5 approximation (x <= 0 ideally)
    auto exp_vals = approx_exp_sqrtexp_deg5(shifted);
    clip_batch_check("softmax after approx exp");
    ...
    // Step 4: Inverse Sum via Newton on scaled sum
    ...
    clip_batch_check("softmax after inv");
...
int main(int argc, char **argv) {
    ...
    const u64 batch = 1;
    // Match unclip_img2img.py config defaults (SD2 UnCLIP small).
    const u64 img_H = 224;
    const u64 img_W = 224;
    const u64 C = 3;
    const u64 vae_scale_factor = 8;
    const u64 out_H = 512;
    const u64 out_W = 512;
    const u64 H = out_H / vae_scale_factor; // latent H
    const u64 W = out_W / vae_scale_factor; // latent W
    const u64 latent_C = 4;
    const u64 seq_len = 77;
    const u64 hidden = 1024;
    const u64 ctx_dim = 1024;
    ...
    const u64 class_labels_dim = img_emb_dim * 2;
    ...
    if (party == SERVER || party == DEALER) {
        ...
        fill_normal(img_pos_embed, stddev);
    ...
    if (party == CLIENT || party == DEALER) {
        ...
        if (!ok || image_values.size() < needed) {
            ...
            for (size_t i = 0; i < needed; ++i) image_values[i] = qrand_uniform_scaled(f, 255.0);
        }
    ...
    auto unet_forward = [&](const AuthShare &lat_in, const AuthShare &prompt_ctx,
                            const AuthShare &cls_labels, int timestep) -> AuthShare {
        u64 B = 2 * batch;
        u64 Ht = H;
        u64 Wt = W;
    ...
    for (int step = 0; step < num_inference_steps; ++step) {
        ...
        auto noise_pred = unet_forward(latents_cfg, prompt_embeds_cfg, class_labels_cfg, t);
