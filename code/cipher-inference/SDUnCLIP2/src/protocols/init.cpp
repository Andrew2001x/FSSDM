    ...
    namespace protocols {
        ...
        namespace init {
            ...
            void eval(int _party, std::string ip, int port, bool oneShot)
            {
                ...
                if (std::getenv("SHARK_PUBLIC_BITKEY"))
                {
                    // Reconstruct full boolean MAC key (bit_key) by XOR.
                    u64 my_bit = (u64)bit_key;
                    u64 peer_bit = 0;
                    if (party == SERVER)
                    {
                        peer->send(my_bit);
                        peer_bit = peer->recv<u64>();
                    }
                    else
                    {
                        peer_bit = peer->recv<u64>();
                    ...
                    bit_key = (u64)(my_bit ^ peer_bit);
                }
                if (std::getenv("SHARK_DEBUG_RINGKEY"))
                {
                    printf("[debug] ring_key hi=%llu low=%llu\n",
                           (unsigned long long)getHigh(ring_key),
                           (unsigned long long)getLow(ring_key));
                }
                if (std::getenv("SHARK_DEBUG_BITKEY"))
                {
                    printf("[debug] bit_key=%llu\n", (unsigned long long)bit_key);
                }
