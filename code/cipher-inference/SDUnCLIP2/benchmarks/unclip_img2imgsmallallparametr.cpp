...
static void keygen_configure_progress(int num_inference_steps) {
    ...
    keygen_layer_total[1] = 4;  // image encoder, unet core, unet mid attn, vae
    ...
    keygen_layer_total[6] = 5;  // vae post, res1, attn, res2, out
...
static AuthShare silu_apply(const AuthShare &x) {
    ...
    // 5th-order polynomial coefficients for gate = sigmoid(x), on the negative side:
    // outer in [-7, -2.2), inner in [-2.2, 0).
    const auto cO0 = make_public_const(x.share.size(), 5.90010486e-01, f);
    const auto cO1 = make_public_const(x.share.size(), 4.21319936e-01, f);
    const auto cO2 = make_public_const(x.share.size(), 1.26610092e-01, f);
    const auto cO3 = make_public_const(x.share.size(), 1.97985686e-02, f);
    const auto cO4 = make_public_const(x.share.size(), 1.59538912e-03, f);
    const auto cO5 = make_public_const(x.share.size(), 5.25625056e-05, f);

    const auto cI0 = make_public_const(x.share.size(), 4.99957471e-01, f);
    const auto cI1 = make_public_const(x.share.size(), 2.49155471e-01, f);
    const auto cI2 = make_public_const(x.share.size(), -4.01655846e-03, f);
    const auto cI3 = make_public_const(x.share.size(), -2.84974728e-02, f);
    const auto cI4 = make_public_const(x.share.size(), -6.76484741e-03, f);
    const auto cI5 = make_public_const(x.share.size(), -4.34163661e-04, f);
    ...
    auto one = make_public_const(x.share.size(), 1.0, f);
    auto segL = zero;                           // x < -7
    auto segA = p_outer;                        // [-7, -2.2)
    auto segB = p_inner;                        // [-2.2, 0)
    auto segC = ADD_CALL(one, neg_span(p_inner_neg));  // [0, 2.2)
    auto segD = ADD_CALL(one, neg_span(p_outer_neg));  // [2.2, 7)
    auto segR = one;                            // x >= 7
    ...
    update(gate, lt_neg7, segL);
    update(gate, ge_neg2_2, segB);
    update(gate, ge_0, segC);
    update(gate, ge_2_2, segD);
    update(gate, ge_7, segR);

    // Clamp gate to [0, 1].
    auto lt0 = cmp_lt(gate, 0.0);
    auto gt1 = cmp_gt(gate, 1.0);
    update(gate, lt0, segL);
    update(gate, gt1, segR);

    return LRS_CALL(MUL_CALL(x, gate), f);
}
...
// Approximate 1/t with two Newton-Raphson steps, matching:
//   r = 1; r = r * (2 - t * r); r = r * (2 - t * r).
static AuthShare reciprocal_newton(const AuthShare &t, int fp_bits, int nr_iters = 2) {
    auto r = make_public_const(t.share.size(), 1.0, fp_bits);
    auto two = make_public_const(t.share.size(), 2.0, fp_bits);
    for (int iter = 0; iter < nr_iters; ++iter) {
        auto tr = LRS_CALL(MUL_CALL(t, r), fp_bits);
        auto correction = ADD_CALL(two, neg_span(tr));
        r = LRS_CALL(MUL_CALL(r, correction), fp_bits);
    }
    return r;
}

static AuthShare ss_reciprocal(const AuthShare &x) {
    return reciprocal_newton(x, f, 2);
}
...
static AuthShare inv_newton2(const AuthShare &t) {
    return reciprocal_newton(t, f, 2);
}
